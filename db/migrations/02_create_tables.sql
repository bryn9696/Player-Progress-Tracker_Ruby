CREATE TABLE users (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), username VARCHAR(60) UNIQUE, password VARCHAR(60), email VARCHAR(70) UNIQUE, phone_number BIGINT);
...
CREATE TABLE my_teams (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), my_teams_id INTEGER REFERENCES my_teams(id), team_name VARCHAR(60), number_of_players INTEGER, coach_or_manager VARCHAR(60));
